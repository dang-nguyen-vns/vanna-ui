...
let apiStatus = { status: "NotRequested" };
onMount(async () => {
  try {
    apiStatus = { status: "Loading" };
    const queryString = Object.entries($$props).filter(([key, _]) => key !== "endpoint" && key !== "addMessage").map(
...
{:else if apiStatus.status === "Loading"}
  ...
  >
    ...
    <div class="space-y-3">
      <h2 class="font-medium text-gray-800 dark:text-white">Loading...</h2>
    </div>
  </li>
